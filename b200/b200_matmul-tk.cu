    ...
    if(warpgroupid == NUM_CONSUMERS) { // PRODUCER GROUP. NOT CONSUMERS.
        ...
        if(warp::laneid() == 0 && warpgroup::warpid() == 3) {
            ...
            for(int task_iter = 0; true; task_iter++) {
                ...
                mm2_ABt(d_tt, a_smem[input_ring][warpgroup::warpid()], b_smem[input_ring], inputs_finished[input_ring]); // This is the first one so we just use a normal MM to first write the results  
                input_ring=prototype::ring_advance<PIPE_DEPTH>(input_ring); // ????
                for(int idx = 1; idx < iters_per_task; idx++) { // This goes through the remaining threads and then uses the mma (accumulate) function to accumulate to the result
        ...
        for(int task_iter = 0; true; task_iter++) {
            ...
            warpgroup::store(d_smem, d_reg[0]); //move from TMEM to SMEM
